...
def getLESSGroup():
    ...
    linkName = indexhtml.split("less ")[1].split(" -->")[0]
    print "compiling block: " + linkName
    print "-------------------"
    
    start = indexhtml.find('<!-- less')
    end = indexhtml.find('/less -->') + 10
    linkIndex = start
...
# 	get link tag groups
	while(indexhtml.find('<!-- less') != -1):
		compiled.append(getLESSGroup())
		
	clean()

# 	todo - insert new link tags in production.html
